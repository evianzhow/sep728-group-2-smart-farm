...
export default {
  ...
  data() {
    return {
      history: [], // To store the fan history data
    };
  },
  methods: {
    async fetchFanHistory() {
      try {
        ...
        console.log("Fan History Response:", response.data.data);
        ...
        console.error("Error fetching fan history:", error);
      }
    },
    goHome() {
  ...
  mounted() {
    this.fetchFanHistory(); // Fetch fan history data when the component mounts
  },
};
</script>
