...
export default {
  ...
  data() {
    return {
      history: [], // To store the LCD history data
    };
  },
  methods: {
    async fetchLCDHistory() {
      try {
        const token = await this.getAuthToken(); // Ensure you have a valid token
        const response = await axios.get(
          "https://gorgeous-glowworm-definite.ngrok-free.app/controllers/lcd/history",
          {
        ...
        console.log("LCD History Response:", response.data.data);
        ...
        console.error("Error fetching LCD history:", error);
      }
    },
    goHome() {
  ...
  mounted() {
    this.fetchLCDHistory(); // Fetch LCD history data when the component mounts
  },
};
</script>
